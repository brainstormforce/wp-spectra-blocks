...
class Spectra_Post_Assets {
	...
	public function __construct( $post_id ) {
		...
		$this->load_uag_fonts = apply_filters( 'spectra_enqueue_google_fonts', $this->load_uag_fonts );

		if ( $this->preview || $this->is_post_revision ) {
			$this->file_generation              = 'disabled';
			$this->is_allowed_assets_generation = true;
		} else {
			$this->file_generation              = Spectra_Helper::$file_generation;
			$this->is_allowed_assets_generation = $this->allow_assets_generation();
		}
		// Set other options.
		$this->load_gfonts_locally = Spectra_Admin_Helper::get_admin_settings_option( 'uag_load_gfonts_locally', 'disabled' );
		$this->preload_local_fonts = Spectra_Admin_Helper::get_admin_settings_option( 'uag_preload_local_fonts', 'disabled' );
	...
	public function spectra_gbs_load_styles() {
		...
		$gbs_status                  = \Spectra_Admin_Helper::get_admin_settings_option( 'uag_enable_gbs_extension', 'enabled' );
		...
		if ( 'disabled' === $gbs_status ) {
			...
			foreach ( $spectra_global_block_styles as $style ) {
				...
				if ( file_exists( $file_dir ) ) {
					...
					wp_enqueue_style( $file_names, $file_url, array(), SPECTRA_VER, 'all' );
				}
			}
		}
			...
			if ( is_array( $style ) ) {
				foreach ( $style as $family ) {
					if ( ! in_array( $family, $families, true ) ) {
						Spectra_Helper::blocks_google_font( true, $family, '' );
						$families[] = $family;
					}
				}
			}
	...
	public function allow_assets_generation() {
		...
		if ( SPECTRA_ASSET_VER !== $page_assets['uag_version'] ) {
		...
		if ( 'enabled' === $this->file_generation ) {
			...
			if ( ! empty( $css_file_name ) ) {
				$css_asset_info = Spectra_Scripts_Utils::get_asset_info( 'css', $this->post_id );
				$css_file_path  = $css_asset_info['css'];
			}

			if ( ! empty( $js_file_name ) ) {
				$js_asset_info = Spectra_Scripts_Utils::get_asset_info( 'js', $this->post_id );
	...
	public function enqueue_scripts() {
		...
		$blocks = array();
		if ( Spectra_Admin_Helper::is_block_theme() ) {
		...
		if ( $this->uag_flag ) {
			...
			if ( 'disabled' === $this->file_generation || $this->fallback_css ) {
				Spectra_Scripts_Utils::enqueue_blocks_styles(); // Enqueue block styles.
	...
	public function enqueue_blocks_dependency_frontend() {
		...
		$spectra_grid_ajax_nonce    = wp_create_nonce( 'spectra_grid_ajax_nonce' );
		wp_localize_script(
			'uagb-post-js',
			'spectra_data',
			array(
				...
				'spectra_grid_ajax_nonce'    => $spectra_grid_ajax_nonce,
			)
		);
	...
	public function enqueue_file_generation_assets() {
		...
		*/
		$spectra_asset_ver = apply_filters( 'spectra_asset_version', SPECTRA_ASSET_VER );

		if ( empty( $spectra_asset_ver ) || ! is_string( $spectra_asset_ver ) ) {
			$spectra_asset_ver = SPECTRA_ASSET_VER;
		}

		if ( isset( $file_handler['css_url'] ) ) {
			wp_enqueue_style( 'uag-style-' . $this->post_id, $file_handler['css_url'], array(), $spectra_asset_ver, 'all' );
		} else {
			$this->fallback_css = true;
		}
		if ( isset( $file_handler['js_url'] ) ) {
			wp_enqueue_script( 'uag-script-' . $this->post_id, $file_handler['js_url'], array(), $spectra_asset_ver, true );
		} else {
	...
	public function print_conditional_css() {
		...
		$conditional_block_css = Spectra_Block_Helper::get_condition_block_css();

		if ( in_array( 'uagb/masonry-gallery', $this->current_block_list, true ) ) {
			$conditional_block_css .= Spectra_Block_Helper::get_masonry_gallery_css();
		}
	...
	public function generate_fonts() {
		...
		if ( isset( $fonts_attr ) && ! empty( $fonts_attr ) ) {
			...
			if ( 'enabled' === $this->load_gfonts_locally ) {
				...
				require_once SPECTRA_DIR . 'lib/uagb-webfont/uagb-webfont-loader.php';

				// link with https protocol to download fonts.
				$fonts_link = 'https:' . $fonts_link;

				$fonts_data = spectra_get_webfont_remote_styles( $fonts_link );

				$this->stylesheet = $fonts_data . $this->stylesheet;

				if ( 'enabled' === $this->preload_local_fonts ) {

					$font_files = spectra_get_preload_local_fonts( $fonts_link );
	...
	public function render_google_fonts() {
		...
		$show_google_fonts = apply_filters( 'spectra_blocks_show_google_fonts', true );
		...
		if ( 'disabled' === $this->load_gfonts_locally ) {
			...
			wp_enqueue_style( 'uag-google-fonts-' . $this->post_id, $this->gfonts_url, array(), SPECTRA_VER, 'all' );
	...
	public function get_block_css_and_js( $block ) {
		...
		if ( isset( $block['attrs'] ) && is_array( $block['attrs'] ) ) {
			 ...
			 */
			$blockattr = apply_filters( 'spectra_block_attributes_for_css_and_js', $block['attrs'], $name );
		...
		if ( 'core/gallery' === $name && isset( $block['attrs']['masonry'] ) && true === $block['attrs']['masonry'] ) {
			...
			$this->uag_flag             = true;
			$css                       += Spectra_Block_Helper::get_gallery_css( $blockattr, $block_id );
		}
		...
		if ( strpos( $name, 'uagb/' ) !== false ) {
			...
			$_block_css = Spectra_Block_Module::get_frontend_css( $_block_slug, $blockattr, $block_id );
			$_block_js  = Spectra_Block_Module::get_frontend_js( $_block_slug, $blockattr, $block_id, 'js' );
	...
	public function common_function_for_assets_preparation( $post_content ) {
		...
		if ( 'yes' === $enable_on_page_css_button ) {
			...
			if ( ! empty( $custom_css ) && ! self::$custom_css_appended ) {
				$this->stylesheet         .= Spectra_Admin_Helper::sanitize_inline_css( $custom_css );
		...
		// Update fonts.
		$this->gfonts = array_merge( $this->gfonts, Spectra_Helper::$gfonts );
	}
	...
	public function create_file( $file_data, $type, $file_state = 'new', $old_file_name = '' ) {
		...
		$folder_name    = Spectra_Scripts_Utils::get_asset_folder_name( $this->post_id );
	...
	public function file_write( $file_data, $type = 'css', $post_id = 0 ) {
		...
		$assets_info = Spectra_Scripts_Utils::get_asset_info( $type, $this->post_id );
		...
		if ( '' === $file_name ) {
			...
			if ( $did_create ) {
				$new_assets_info           = Spectra_Scripts_Utils::get_asset_info( $type, $this->post_id );
	...
	public function get_block_static_css( $block_name ) {
		...
		$block_static_css_path = SPECTRA_DIR . 'assets/css/blocks/' . $block_name . '.css';
