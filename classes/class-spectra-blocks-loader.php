...
use Spectra\Admin_Helper;
use \ZipAI\Classes\Module as Zip_Ai_Module;
use SpectraBlocksAdmin\Inc\Admin_Learn;

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly.
}

if ( ! class_exists( 'Spectra_Loader' ) ) {

	/**
	 * Class Spectra_Loader.
	 */
	final class Spectra_Loader {
		...
		public function __construct() {
			...
			// Activation hook.
			register_activation_hook( SPECTRA_FILE, array( $this, 'activation_reset' ) );

			// deActivation hook.
			register_deactivation_hook( SPECTRA_FILE, array( $this, 'deactivation_reset' ) );
			...
			*/
			if ( ! class_exists( 'BSF_Analytics_Loader' ) ) {
				require_once SPECTRA_DIR . 'lib/bsf-analytics/class-bsf-analytics-loader.php';
			}
				...
				$spectra_bsf_analytics->set_entity(
					array(
						'spectra-blocks' => array(
							...
							'path'                => SPECTRA_DIR . 'lib/bsf-analytics',
			...
			$this->block_analytics = Spectra_Block_Analytics::get_instance();
		}
		...
		public function define_constants() {
			define( 'SPECTRA_BASE', plugin_basename( SPECTRA_FILE ) );
			define( 'SPECTRA_DIR', plugin_dir_path( SPECTRA_FILE ) );
			define( 'SPECTRA_URL', plugins_url( '/', SPECTRA_FILE ) );
			define( 'SPECTRA_VER', '0.0.1' );
			define( 'SPECTRA_MODULES_DIR', SPECTRA_DIR . 'modules/' );
			define( 'SPECTRA_MODULES_URL', SPECTRA_URL . 'modules/' );
			define( 'SPECTRA_SLUG', 'spectra-blocks' );
			define( 'SPECTRA_URI', trailingslashit( 'https://wpspectra.com/' ) );

			if ( ! defined( 'SPECTRA_TABLET_BREAKPOINT' ) ) {
				define( 'SPECTRA_TABLET_BREAKPOINT', '976' );
			}
			if ( ! defined( 'SPECTRA_MOBILE_BREAKPOINT' ) ) {
				define( 'SPECTRA_MOBILE_BREAKPOINT', '767' );
			}
			...
			if ( ! defined( 'SPECTRA_UPLOAD_URL' ) ) {
				define( 'SPECTRA_UPLOAD_URL', $upload_dir['baseurl'] . '/' . SPECTRA_UPLOAD_DIR_NAME . '/' );
			}

			define( 'SPECTRA_ASSET_VER', get_option( '__uagb_asset_version', SPECTRA_VER ) );
			define( 'SPECTRA_CSS_EXT', defined( 'WP_DEBUG' ) && WP_DEBUG ? '.css' : '.min.css' );
			define( 'SPECTRA_JS_EXT', defined( 'WP_DEBUG' ) && WP_DEBUG ? '.js' : '.min.js' );
		}
		...
		require_once SPECTRA_DIR . 'classes/class-spectra-blocks-scripts-utils.php';
			...
			$enable_templates_button = Spectra_Admin_Helper::get_admin_settings_option( 'uag_enable_templates_button', 'yes' );
		...
		public function load_textdomain() {
			...
			$lang_dir = SPECTRA_DIR . 'languages/';
			...
			$mofile = '';
			if ( is_string( $locale ) ) {
				$mofile = sprintf( '%1$s-%2$s.mo', 'spectra-blocks', $locale );
			}
			...
			if ( file_exists( $mofile_global ) ) {
				...
				load_plugin_textdomain( 'spectra-blocks', false, $lang_dir );
			}
		}
		 ...
		 * @return void
		 */
		public function spectra_fails_to_load() {
		...
		public function init_actions() {
			...
			if ( ! $this->is_gutenberg_active() ) {
				add_action( 'admin_notices', array( $this, 'spectra_fails_to_load' ) );
				return;
			}
			...
			register_meta(
				...
				array(
					...
					'sanitize_callback' => function( $meta_value ) {
						return Spectra_Admin_Helper::sanitize_inline_css( $meta_value );
					},
				)
			);

			// This class is loaded from blocks config.
			Spectra_Popup_Builder::generate_scripts();

			Spectra_Update::migrate_visibility_mode();
		...
		public function get_regenerate_assets_on_migration() {
			...
			$stored_domain = \Spectra_Admin_Helper::get_admin_settings_option( 'spectra_site_url', '' );

			// If the stored domain is empty, update the 'spectra_site_url' option in admin settings with the modified site domain and return.
			if ( empty( $stored_domain ) ) {
				\Spectra_Admin_Helper::update_admin_settings_option( 'spectra_site_url', $site_domain );
				return;
			}
		...
		public function create_block_status_array() {
			$saved_blocks      = (array) \Spectra_Admin_Helper::get_admin_settings_option( '_uagb_blocks' );
			$block_manager     = spectra_block();
		...
		public function global_settings_data() {
			...
			$global_data['internal_referer']              = isset( $bsf_internal_referrer['spectra-blocks'] ) 
				? $bsf_internal_referrer['spectra-blocks'] 
				: '';
			$global_data['beta']                          = get_option( 'spectra_beta' );
		...
		public function spectra_get_specific_stats( $default_stats ) {
			...
			if ( empty( $default_stats['plugin_data']['spectra-blocks'] ) || ! is_array( $default_stats['plugin_data']['spectra-blocks'] ) ) {
				$default_stats['plugin_data']['spectra-blocks'] = array();
			}
			...
			$default_stats['plugin_data']['spectra-blocks'] = array_merge_recursive( $default_stats['plugin_data']['spectra-blocks'], $array_of_defaults );
			$default_stats['plugin_data']['spectra-blocks'] = array_merge_recursive( $default_stats['plugin_data']['spectra-blocks'], $this->global_settings_data() );
			$default_stats['plugin_data']['spectra-blocks'] = array_merge_recursive( $default_stats['plugin_data']['spectra-blocks'], $this->create_block_status_array() );

			// Add advanced block usage statistics.
			if ( is_object( $this->block_analytics ) ) {
				$default_stats['plugin_data']['spectra-blocks'] = $this->block_analytics->get_block_stats_for_analytics( $default_stats['plugin_data']['spectra-blocks'] );
			}

			// Add learn progress analytics data.
			self::add_learn_progress_analytics_data( $default_stats['plugin_data']['spectra-blocks'] );
...
/**
 *  Prepare if class 'Spectra_Loader' exist.
 *  Kicking this off by calling 'get_instance()' method
 */
Spectra_Loader::get_instance();
 ...
 */
function spectra() {
	return Spectra_Loader::get_instance();
}
