...
if ( ! class_exists( 'Spectra_Admin_Helper' ) ) {
	 ...
	 */
	final class Spectra_Admin_Helper {
		...
		public static function get_admin_settings_shareable_data() {
			...
			$options = array(
				'spectra_beta'                         => self::get_admin_settings_option( 'spectra_beta', 'no' ),
				...
				'uag_blocks_editor_spacing'         => apply_filters( 'spectra_default_blocks_editor_spacing', self::get_admin_settings_option( 'uag_blocks_editor_spacing', 0 ) ),
		...
		public static function get_block_options() {
			...
			$blocks       = Spectra_Helper::$block_list;
			$saved_blocks = self::get_admin_settings_option( '_uagb_blocks' );
		error_log('Spectra_Admin_Helper::get_block_options() - Spectra_Helper::$block_list has ' . (is_array($blocks) ? count($blocks) : 'null/not-array') . ' blocks');
			...
			return apply_filters( 'spectra_enabled_blocks', Spectra_Helper::$block_list );
		}
		...
		public static function create_specific_stylesheet() {
			...
			foreach ( $combined as $key => $c_block ) {
				...
				if ( false !== strpos( $c_block, '-pro' ) ) {
					...
					$style_file = SPECTRA_DIR . 'assets/css/blocks/' . $c_block . '.css';
				}
		...
		public function get_rollback_versions() {
			...
			$rollback_versions = get_transient( 'uag_rollback_versions_' . SPECTRA_VER );
			...
			if ( empty( $rollback_versions ) ) {
				...
				foreach ( $plugin_information->versions as $version => $download_link ) {
					...
					if ( version_compare( $version, SPECTRA_VER, '>=' ) ) {
						continue;
					}
				...
				set_transient( 'uag_rollback_versions_' . SPECTRA_VER, $rollback_versions, WEEK_IN_SECONDS );
			}
		...
		public static function get_global_content_width() {
			...
			if ( '' === $content_width ) {
				...
				if ( self::is_block_theme() ) {
					...
					$uag_content_width_set_by = __( "Full Site Editor's Global Styles", 'spectra' );
				} elseif ( 'default' !== $content_width_third_party ) {
					$content_width            = intval( $content_width_third_party );
					$uag_content_width_set_by = __( 'Filter added through any 3rd Party Theme/Plugin.', 'spectra' );
					...
					$uag_content_width_set_by = $ast_theme_name . ' ' . __( 'Theme', 'spectra' );
				}
			}
		...
		public static function get_spectra_pro_url( $path, $source = '', $medium = '', $campaign = '' ) {
			...
			if ( class_exists( '\BSF_UTM_Analytics\Inc\Utils' ) && is_callable( '\BSF_UTM_Analytics\Inc\Utils::get_utm_ready_link' ) ) {
				$spectra_pro_url = \BSF_UTM_Analytics\Inc\Utils::get_utm_ready_link( $spectra_pro_url, 'spectra' );
			} else {
				if ( ! empty( $source ) ) {
	...
	/**
	 *  Prepare if class 'Spectra_Admin_Helper' exist.
	 *  Kicking this off by calling 'get_instance()' method
	 */
	Spectra_Admin_Helper::get_instance();
}
