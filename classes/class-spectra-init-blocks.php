...
class Spectra_Init_Blocks {
	...
	public function get_cf7_forms() {
		...
		if ( empty( $field_options ) ) {
			$field_options = array(
				'-1' => __( 'You have not added any Contact Form 7 yet.', 'spectra' ),
			);
		}
		return $field_options;
	}
	...
	public function get_gravity_forms() {
		...
		if ( empty( $field_options ) ) {
			$field_options = array(
				'-1' => __( 'You have not added any Gravity Forms yet.', 'spectra' ),
			);
		}
	...
	public function uag_global_sidebar_enabled() {
		...
		if ( ! empty( $_POST['enableQuickActionSidebar'] ) ) {
			...
			\Spectra_Admin_Helper::update_admin_settings_option( 'uag_enable_quick_action_sidebar', $spectra_enable_quick_action_sidebar );
			wp_send_json_success();
		}
		wp_send_json_error();
	}
	...
	public function uag_global_update_allowed_block() {
		...
		if ( ! empty( $_POST['defaultAllowedQuickSidebarBlocks'] ) ) {
			...
			\Spectra_Admin_Helper::update_admin_settings_option( 'spectra_quick_sidebar_allowed_blocks', $spectra_default_allowed_quick_sidebar_blocks );
			wp_send_json_success();
		}
		wp_send_json_error();
	}
	...
	public function uag_global_block_styles() {
		...
		$response_data = array( 'messsage' => __( 'No post data found!', 'spectra' ) );
		...
		foreach ( $global_block_styles as $key => $style ) {
			if ( ! empty( $_POST['globalBlockStyleId'] ) && ! empty( $style['value'] ) && $style['value'] === $_POST['globalBlockStyleId'] ) {
				...
				$_block_css  = Spectra_Block_Module::get_frontend_css( $_block_slug, $block_attr, $block_attr['block_id'], true );
				...
				if ( ! empty( $global_block_styles[ $key ]['post_ids'] ) && is_array( $global_block_styles[ $key ]['post_ids'] ) ) {
					foreach ( $global_block_styles[ $key ]['post_ids'] as $post_id ) {
						Spectra_Helper::delete_page_assets( $post_id );
					}
				}
			}
		}
