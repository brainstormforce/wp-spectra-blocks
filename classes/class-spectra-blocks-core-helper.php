...
if ( ! class_exists( 'Spectra_Helper' ) ) {

	/**
	 * Class Spectra_Helper.
	 */
	final class Spectra_Helper {
		...
		/**
		 * Get Json Data.
		 * Customize and add icons via 'spectra_icons_chunks' filter.
		 *
		 * @since 0.0.1
		 * @return array
		 */
		public static function backend_load_font_awesome_icons() {
			...
			$icons_chunks = apply_filters( 'spectra_icons_chunks', $icons_chunks );
		...
		public static function render_svg_html( $icon ) {
			...
			if ( 'disabled' !== $load_font_awesome_5 ) {
				...
				$font_awesome_5_polyfiller = spectra_blocks_get_font_awesome_polyfiller();
		...
		public static function get_query( $attributes, $block_type ) {
			$fallback_for_posts_to_show = Spectra_Block_Helper::get_fallback_number( $attributes['postsToShow'], 'postsToShow', $attributes['blockName'] );
			$fallback_for_offset        = Spectra_Block_Helper::get_fallback_number( $attributes['postsOffset'], 'postsOffset', $attributes['blockName'] );
			...
			$query_args = apply_filters( "spectra_post_query_args_{$block_type}", $query_args, $attributes );
		...
		public static function get_image_sizes() {
			...
			$image_sizes = apply_filters( 'spectra_post_featured_image_sizes', $image_sizes );
		...
		public static function get_upload_dir() {
			...
			// Build the paths.
			$dir_info = array(
				'path' => trailingslashit( trailingslashit( $wp_info['basedir'] ) . SPECTRA_UPLOAD_DIR_NAME ),
				'url'  => trailingslashit( trailingslashit( $wp_info['baseurl'] ) . SPECTRA_UPLOAD_DIR_NAME ),
			);
		...
		public static function allow_file_generation() {
			return apply_filters( 'spectra_allow_file_generation', get_option( '_uagb_allow_file_generation', 'disabled' ) );
		}
		...
		public function get_block_css_and_js( $block ) {
			...
			if ( strpos( $name, 'uagb/' ) !== false ) {
				// V2 blocks removed - no CSS/JS generation needed
				// V3 blocks use 'spectra/' namespace and handle assets differently
				self::$uag_flag = true;
		...
		public function generate_assets() {
			...
			if ( is_single() || is_page() || is_404() ) {
				 ...
				 */
				$this_post = apply_filters( 'spectra_post_for_stylesheet', $this_post );
		 ...
		 * @since 0.0.1
		 */
		public static function spectra_get_excerpt( $post_id, $content, $length_fallback ) {
		...
		public static function delete_page_assets( $post_id ) {
			...
			do_action( 'spectra_delete_page_assets' );
		}
	...
	/**
	 *  Prepare if class 'Spectra_Helper' exist.
	 *  Kicking this off by calling 'get_instance()' method
	 */
	Spectra_Helper::get_instance();
}
