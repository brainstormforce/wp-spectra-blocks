 ...
 *
 * Downloads the latest Font Awesome 7.x free icon metadata from GitHub,
...
const FA_ICONS_URL  = 'https://raw.githubusercontent.com/FortAwesome/Font-Awesome/7.x/metadata/icons.json';
...
function buildPhpFile( chunk, index ) {
	const lines = [
		'<?php',
		'/**',
		` * Font Awesome 7 icon data — chunk ${ index } of ${ NUM_CHUNKS }.`,
		' *',
...
async function main() {
	...
	// Retain icons dropped by the current Font Awesome major version so content
	// authored against their names keeps rendering the original glyph. Only
	// merged when FA no longer ships the icon as a primary key.
	const legacyPath = path.join( CONFIG_DIR, 'legacy-icons.json' );
	if ( fs.existsSync( legacyPath ) ) {
		const legacyIcons = JSON.parse( fs.readFileSync( legacyPath, 'utf8' ) );
		let retained = 0;
		for ( const [ iconKey, legacyData ] of Object.entries( legacyIcons ) ) {
			if ( processed[ iconKey ] ) {
				continue; // still shipped by FA — nothing to retain
			}
			const entry = {
				label            : legacyData.label ?? iconKey,
				styles           : legacyData.styles ?? [],
				search           : { terms: legacyData.search?.terms ?? [] },
				custom_categories: [],
				svg              : legacyData.svg ?? {},
			};
			if ( Object.keys( entry.svg ).length === 0 ) {
		...
		if ( retained ) {
			console.log( `→ Retained ${ retained } legacy icon(s) dropped by Font Awesome` );
		}
	}
	...
	// Legacy alias index (FA4/old name => FA7 primary key). The chunks key
	...
	const aliasLines = [
		...
		'',
		'/**',
		' * Font Awesome 7 icon-name aliases (legacy/FA4 name => FA7 primary key).',
		' *',
