...
class BlockUsageTracker {
	 ...
	 *
	 * Called when a post is saved but no longer contains any Spectra blocks.
	 *
	 * @since x.x.x
	 *
	 * @param int $post_id Post ID.
	 */
	private function remove_post_from_usage_data( $post_id ) {
		...
		// Remove the post data from usage_data.
		unset( $analytics_data['usage_data'][ $post_id ] );

		// Update the total posts count in statistics.
		...
		// Invalidate cache when data is removed.
		$this->clear_analytics_cache();
	}
	 ...
	 */
	private function count_valid_posts( $usage_data ) {
	 ...
	 *
	 * Cleans up the usage_data and recalculates statistics when a post is deleted.
	 *
	 * @since x.x.x
	 *
	 * @param int $post_id Post ID being deleted.
	 */
	public function remove_post_analytics_data( $post_id ) {
		// Skip if user has not opted in for analytics.
		if ( ! $this->is_analytics_enabled() ) {
			return;
		}
	 ...
	 *
	 * Marks the post as trashed in analytics data so it's excluded from active count.
	 *
	 * @since x.x.x
	 *
	 ...
	 *
	 * Removes the trashed flag from analytics data so it's included in active count again.
	 *
	 * @since x.x.x
	 *
	...
	private function get_total_posts_with_spectra_3_blocks() {
		...
		// Count only non-trashed/non-deleted posts.
		$count = 0;
		foreach ( $usage_data as $post_id => $post_data ) {
	...
	public function get_site_activity_level() {
		...
		// Count posts with Spectra blocks edited in the last 180 days.
		...
		foreach ( $usage_data as $post_id => $post_data ) {
			...
			// Verify post still exists and is published (not trashed/deleted).
			$post_status = get_post_status( $post_id );
			if ( false === $post_status || 'trash' === $post_status ) {
				continue;
			}
			...
			// Check if the post was updated within the threshold period.
