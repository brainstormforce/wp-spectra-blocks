...
module.exports = function ( grunt ) {
		...
		copy: {
			main: {
				...
				src: [
					...
					'!auth.json',
					'!spectra-v3/node_modules/**',
					'!spectra-v3/src/**',
					'!spectra-v3/composer.json',
					'!spectra-v3/composer.lock',
					'!spectra-v3/package.json',
					'!spectra-v3/package-lock.json',
					'!spectra-v3/README.md',
				],
	...
	// Process new FontAwesome 6.7.2 icons.
	grunt.registerTask( 'font-awesome-v6-7-2', function () {
		const done = this.async();
		const getCategories = grunt.file.readJSON( './bin/icons-configure/fontawesome-category.json' );
		const getCategoriesCustomTitle = grunt.file.readJSON( './bin/icons-configure/fontawesome-custom-shorted-category.json' );
		// This is custom category list like in our icon library brand category is not available so we put manually here.
		const getCustomCategoryTitle = grunt.file.readJSON( './bin/icons-configure/custom-icon-category.json' );
		let getDownloadedIcons = grunt.file.readJSON( './bin/icons-configure/spectra-icons-v6-7-2.json' );
		const fs = require( 'fs' );
		...
		fs.writeFile( configFilePath,
			...
			function ( err ) {
				if ( ! err ) {
					console.log('Font-Awesome v6 new icons chunk 4 library updated!'); // eslint-disable-line
				}
				done();
			}
		);
	...
	// Single command to update new FontAwesome 6.7.2 icons.
	grunt.registerTask( 'font-awesome-v6-7-2-php-array-update', [ 'font-awesome-v6-7-2', 'json2php', 'clean-icon-svg-json-file' ] );
};
