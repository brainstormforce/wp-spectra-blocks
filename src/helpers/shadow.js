...
export const parseShadowString = ( shadowString ) => {
	...
	// Extract color - handle CSS variables, hex, rgb, rgba, hsl, hsla
	let color = '';

	// Match CSS variables: var(--something) or var(--something, fallback)
	const varMatch = cleanString.match( /var\([^)]+\)/i );
	if ( varMatch ) {
		color = varMatch[ 0 ];
		cleanString = cleanString.replace( color, '' ).trim();
	}

	// Match rgba/hsla with spaces inside parentheses: rgba(0, 0, 0, 0.5)
	if ( ! color ) {
		const rgbaMatch = cleanString.match( /(?:rgba?|hsla?)\([^)]+\)/i );
		if ( rgbaMatch ) {
			color = rgbaMatch[ 0 ];
			cleanString = cleanString.replace( color, '' ).trim();
		}
	...
	if ( ! color ) {
		const hexMatch = cleanString.match( /#[0-9a-fA-F]{3,8}/i );
		if ( hexMatch ) {
			color = hexMatch[ 0 ];
			cleanString = cleanString.replace( color, '' ).trim();
		}
	...
	// Extract numeric values from remaining string
	const parts = cleanString.split( /\s+/ );
	const numericParts = [];

	parts.forEach( part => {
		if ( part.includes( 'px' ) || part.includes( 'rem' ) || part.includes( 'em' ) || /^-?\d+$/.test( part ) ) {
