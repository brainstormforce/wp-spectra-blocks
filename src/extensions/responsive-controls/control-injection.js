...
class ControlInjectionManager {
	...
	isResponsiveControl( control, selectedBlock = null ) {
		...
		// Special handling for container Shape Dividers panel - only include width/height controls (translation-aware).
		if ( currentBlock.name === 'spectra/container' && this.isPanelTitleMatch( panelTitle, 'Shape Dividers' ) ) {
			const shapeDividerInclusions = [ 'Width', 'Height' ];
			const isIncluded = shapeDividerInclusions.some( ( includedControl ) => {
				// Check English version
				if ( allTexts.includes( includedControl.toLowerCase() ) ) {
					return true;
				}
		...
		// Special handling for container Border panel - exclude Border Hover control (translation-aware).
		if ( currentBlock.name === 'spectra/container' && this.isPanelTitleMatch( panelTitle, 'Border' ) ) {
			const containerExclusions = [ 'Border Hover', 'Enable Border Hover' ];
			const isExcluded = containerExclusions.some( ( excludedControl ) => {
				// Check English version.
				if ( allTexts.includes( excludedControl.toLowerCase() ) ) {
					return true;
				}
				// Check translated version (Spectra textdomain for container controls).
				// eslint-disable-next-line @wordpress/i18n-no-variables
