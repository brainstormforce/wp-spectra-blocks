/**
 * Attribute helpers and editor HOCs for the pre-7.1 editor.
 *
 ...
 *
 * Includes `withResponsiveControls` (the `lg`/`md`/`sm` write router) and
 * `withDeviceViewUpdate` (the projection layer that makes core's panels show the
 * previewed device). The 7.1 path deleted the latter — see #732, b2da4f56 —
 * because rewriting stored attributes in order to display something corrupted
 * content. Sites without viewport states have no alternative, so it stays here.
 *
 * @since 1.0.7
 */
...
import { getResetInProgress } from '../../index';
import { storeWithStyleStates } from './style-state-fallback';
...
const normalizeBorderFormat = ( responsiveControls ) => {
	...
	breakpoints.forEach( ( bp ) => {
		...
		if ( hasShorthandBorder ) {
			...
			// Set individual border properties, keeping whatever each side already
			// declares. A breakpoint can hold both shapes at once — per-side colours
			// plus a shorthand left behind by an earlier linked border. Replacing the
			// side outright made all four sides take the shorthand colour the moment
			// that device was previewed, so four distinct side colours collapsed into
			// one. The shorthand may only fill the properties a side does not declare.
			Object.keys( individualBorder ).forEach( ( side ) => {
				const existingSide = normalized[ bp ]?.style?.border?.[ side ];
				let sideValue = individualBorder[ side ];

				if ( existingSide && typeof existingSide === 'object' ) {
					sideValue = { ...sideValue };

					Object.keys( existingSide ).forEach( ( sideProp ) => {
						if ( hasValue( existingSide[ sideProp ] ) ) {
							sideValue[ sideProp ] = existingSide[ sideProp ];
						}
					} );
				}

				setNested( normalized, `${ bp }.style.border.${ side }`, sideValue );
			} );
...
export const withDeviceViewUpdate = createHigherOrderComponent( ( BlockEdit ) => {
	return ( props ) => {
		...
		/*
		 * The attributes as this block was first seen, kept for the desktop layer
		 * of `storeWithStyleStates()`. The live attributes are a PROJECTION of
		 * whichever device is previewed, so by the time Desktop is selected again
		 * they hold Mobile's values — reading the desktop layer from them handed
		 * the block back its own mobile layout.
		 */
		const loadedAttributesRef = useRef( attributes );
		...
		const handleDeviceUpdate = useCallback(
			( newDeviceType ) => {
				...
				/*
				 * The store, plus anything only `style` knows — see
				 * `./style-state-fallback.js`. A block VARIATION declares its
				 * responsive design in the shape the current editor writes, which on
				 * a 7.1 site is `style['@tablet']` / `style['@mobile']`, so a variation
				 * inserted here arrives with an empty store and this projection had
				 * nothing to work with: the Info Bar kept its three-column base layout
				 * in the Mobile preview while the front end, which hydrates the same
				 * states server-side, stacked it into one. Read-only, and the legacy
				 * buckets still win wherever they spoke.
				 */
				const currentResponsiveControls = storeWithStyleStates( currentAttributes, name, loadedAttributesRef.current );
