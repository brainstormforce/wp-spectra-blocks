...
class Admin_Menu {
	...
	public function initialize_hooks() {
		...
		add_filter( 'plugin_action_links_' . SPECTRA_BASE, array( $this, 'add_action_links' ) );

		/* Render admin content view */
		add_action( 'uag_render_admin_page_content', array( $this, 'render_content' ), 10, 2 );

		add_action( 'wp_ajax_uagb_recommended_plugin_activate', array( $this, 'spectra_activate_addon' ) );
	...
	public static function get_bsf_plugins() {
		...
		$plugins = array(
			...
			'astra'                           => array(
				'type'         => 'theme',
				'name'         => esc_html__( 'Astra', 'spectra' ),
				'desc'         => esc_html__( 'Fast and customizable theme for your website.', 'spectra' ),
			...
			'astra-sites/astra-sites.php'     => array(
				'type'         => 'plugin',
				'name'         => esc_html__( 'Starter Templates', 'spectra' ),
				'desc'         => esc_html__( 'Launch websites with AI or ready-made templates.', 'spectra' ),
			...
			'surecart/surecart.php'           => array(
				'type'         => 'plugin',
				'name'         => esc_html__( 'SureCart', 'spectra' ),
				'desc'         => esc_html__( 'Sell your products easily on WordPress.', 'spectra' ),
			...
			'presto-player/presto-player.php' => array(
				'type'         => 'plugin',
				'name'         => esc_html__( 'Presto Player', 'spectra' ),
				'desc'         => html_entity_decode( esc_html__( 'Display seamless & interactive videos.', 'spectra' ) ),
	 ...
	 * @return void
	 */
	public function spectra_activate_addon() {
		...
		if ( isset( $_POST['plugin'] ) ) {
			...
			if ( 'plugin' === $type ) {
				...
				if ( ! current_user_can( 'activate_plugins' ) ) {
					wp_send_json_error( esc_html__( 'Plugin activation is disabled for you on this site.', 'spectra' ) );
				}
				if ( isset( $_POST['slug'] ) ) {
				...
				if ( ! is_wp_error( $activate ) ) {
					...
					do_action( 'spectra_plugin_activated', $plugin );

					wp_send_json_success( esc_html__( 'Plugin Activated.', 'spectra' ) );
				}
			}
			...
			if ( 'theme' === $type ) {
				...
				if ( isset( $_POST['slug'] ) ) {
					...
					if ( ! ( current_user_can( 'switch_themes' ) ) ) {
						wp_send_json_error( esc_html__( 'Theme activation is disabled for you on this site.', 'spectra' ) );
					}
					...
					if ( ! is_wp_error( $activate ) ) {
						...
						do_action( 'spectra_theme_activated', $plugin );

						wp_send_json_success( esc_html__( 'Theme Activated.', 'spectra' ) );
					}
				}
			}
		}
		...
		if ( isset( $type ) ) { 
			if ( 'plugin' === $type ) {
				wp_send_json_error( esc_html__( 'Could not activate plugin. Please activate from the Plugins page.', 'spectra' ) );
			} elseif ( 'theme' === $type ) {
				wp_send_json_error( esc_html__( 'Could not activate theme. Please activate from the Themes page.', 'spectra' ) );
			}
		}
	}
	...
	public function add_action_links( $links ) {
		...
		$free_links = array(
			'<a href="' . $default_url . '">' . __( 'Settings', 'spectra' ) . '</a>',
			'<a href="' . $rollback . '">' . __( 'Rollback', 'spectra' ) . '</a>',
		);

		// Check if Spectra Pro plugin is not active.
		if ( ! is_plugin_active( 'spectra-pro/spectra-pro.php' ) && ! file_exists( SPECTRA_DIR . '../spectra-pro/spectra-pro.php' ) ) {
			$free_links[] = '<a href="' . $spectra_pro . '" target="_blank" rel="noreferrer" class="spectra-plugins-go-pro">' . __( 'Get Spectra Pro', 'spectra' ) . '</a>';
		}
	...
	public function render_free_vs_pro() {
		...
		echo '<h1>' . esc_html__( 'Free vs Pro Features', 'spectra' ) . '</h1>';
		echo '<p>' . esc_html__( 'Here you can compare the features of Free and Pro versions.', 'spectra' ) . '</p>';
	...
	public function styles_scripts() {
		...
		wp_enqueue_style( $admin_slug . '-font', 'https://fonts.googleapis.com/css2?family=Inter:wght@400;500&display=swap', array(), SPECTRA_VER );
		// Styles.
		wp_enqueue_style( $admin_slug . '-menu-style', SPECTRA_URL . 'admin/assets/legacy/spectra-submenu.css', array(), SPECTRA_VER );
		...
		if ( ! defined( 'SPECTRA_URI' ) ) {
			define( 'SPECTRA_URI', trailingslashit( 'https://wpspectra.com/' ) );
		}
		$localize = apply_filters(
			...
			array(
				...
				'plugin_dir'                => SPECTRA_URL,
				'preview_url'               => SPECTRA_URL . 'admin/assets/',
				'plugin_ver'                => SPECTRA_VER,
				...
				'uag_content_width_set_by'  => \Spectra_Admin_Helper::get_admin_settings_option( 'uag_content_width_set_by', __( 'Spectra', 'spectra' ) ),
				'spectra_pro_installed'     => file_exists( SPECTRA_DIR . '../spectra-pro/spectra-pro.php' ),
				'spectra_pro_licensing'     => file_exists( SPECTRA_DIR . '../spectra-pro/admin/license-handler.php' ),
				...
				'spectra_website'           => array(
					'baseUrl'                => SPECTRA_URI,
					'docsUrl'                => \Spectra_Admin_Helper::get_spectra_pro_url( '/docs/', 'free-plugin', 'spectra-dashboard', 'documentation' ),
					'docsCategoryDynamicUrl' => \Spectra_Admin_Helper::get_spectra_pro_url( '/docs-category/{{category}}', 'free-plugin', 'spectra-dashboard', 'documentation' ),
					'vipPrioritySupportUrl'  => \Spectra_Admin_Helper::get_spectra_pro_url( '/vip-priority-support/', 'free-plugin', 'spectra-dashboard', 'vip-priority-support' ),
					'templatesUrl'           => \Spectra_Admin_Helper::get_spectra_pro_url( '/pricing/', 'free-plugin', 'spectra-dashboard', 'starter-templates' ),
					'banner'                 => \Spectra_Admin_Helper::get_spectra_pro_url( '/pricing/', 'free-plugin', 'spectra-dashboard', 'banner' ),
					'topBar'                 => \Spectra_Admin_Helper::get_spectra_pro_url( '/pricing/', 'free-plugin', 'spectra-dashboard', 'top-bar' ),
					'freeVsPro'              => \Spectra_Admin_Helper::get_spectra_pro_url( '/pricing/', 'free-plugin', 'spectra-dashboard', 'free-vs-pro' ),
					'setting'                => \Spectra_Admin_Helper::get_spectra_pro_url( '/pricing/', 'free-plugin', 'spectra-dashboard', 'setting' ),
					'uagDashboard'           => \Spectra_Admin_Helper::get_spectra_pro_url( '/pricing/', 'free-plugin', 'spectra-dashboard', 'uag-dashboard' ),
					'whatsNewFeedUrl'        => esc_url( SPECTRA_URI . '/whats-new/feed/' ),
					'upsellModalAdmin'       => \Spectra_Admin_Helper::get_spectra_pro_url( '/pricing/', 'free-plugin', 'spectra-dashboard', 'upsell-popup-view-plan' ),
				),
				'plugin_installing_text'    => esc_html__( 'Installing', 'spectra' ),
				'plugin_installed_text'     => esc_html__( 'Installed', 'spectra' ),
				'plugin_activating_text'    => esc_html__( 'Activating', 'spectra' ),
				'plugin_activated_text'     => esc_html__( 'Activated', 'spectra' ),
				'plugin_activate_text'      => esc_html__( 'Activate', 'spectra' ),
				...
				'spectra_old_user_less_than_3' => \Spectra_Helper::is_old_user_less_than_v3() ? 'yes' : 'no',
	...
	public function get_blocks_info_for_activation_deactivation() {
		...
		$blocks = \Spectra_Admin_Helper::get_block_options();
