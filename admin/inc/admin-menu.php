...
namespace SpectraAdmin\Inc;

use SpectraAdmin\Inc\Admin_Helper;
...
use \SpectraZipAI\Classes\Helper as Zip_Ai_Helper;
...
class Admin_Menu {
			...
			if ( 'plugin' === $type ) {
				...
				if ( isset( $_POST['slug'] ) ) {
					...
					if ( class_exists( '\BSF_UTM_Analytics\Inc\Utils' ) && is_callable( '\BSF_UTM_Analytics\Inc\Utils::update_referer' ) ) {
						...
						\BSF_UTM_Analytics\Inc\Utils::update_referer( 'spectra', $slug );
					}
				}
	...
	public function add_action_links( $links ) {
		...
		$spectra_blocks_pro = \Spectra_Blocks_Admin_Helper::get_spectra_pro_url( '/pricing/', 'free-plugin', 'plugin-list', 'plugin-list' );

		$free_links = array(
			'<a href="' . $default_url . '">' . __( 'Settings', 'spectra-blocks' ) . '</a>',
			'<a href="' . $rollback . '">' . __( 'Rollback', 'spectra-blocks' ) . '</a>',
		);
		...
		if ( ! is_plugin_active( 'spectra-pro/spectra-pro.php' ) && ! file_exists( SPECTRA_BLOCKS_DIR . '../spectra-pro/spectra-pro.php' ) ) {
			$free_links[] = '<a href="' . $spectra_blocks_pro . '" target="_blank" rel="noreferrer" class="spectra-plugins-go-pro">' . __( 'Get Spectra Pro', 'spectra-blocks' ) . '</a>';
		}
	...
	public function render() {
		...
		include_once SPECTRA_ADMIN_DIR . 'views/admin-base.php';
	}
	...
	public function render_free_vs_pro() {
		...
		echo '<h1>' . esc_html__( 'Free vs Pro Features', 'spectra-blocks' ) . '</h1>';
		echo '<p>' . esc_html__( 'Here you can compare the features of Free and Pro versions.', 'spectra-blocks' ) . '</p>';
	...
	public function render_content( $menu_page_slug, $page_action ) {
		...
		if ( $this->menu_slug === $menu_page_slug ) {
			include_once SPECTRA_ADMIN_DIR . 'views/dashboard-app.php';
		}
	}
	...
	public function styles_scripts() {
		...
		// Styles.
		wp_enqueue_style( $admin_slug . '-menu-style', SPECTRA_BLOCKS_URL . 'admin/assets/legacy/spectra-submenu.css', array(), SPECTRA_BLOCKS_VER );
		...
		$localize = apply_filters(
			...
			array(
				...
				'uag_content_width_set_by'  => \Spectra_Blocks_Admin_Helper::get_admin_settings_option( 'uag_content_width_set_by', __( 'Spectra', 'spectra-blocks' ) ),
				'spectra_blocks_pro_installed'     => file_exists( SPECTRA_BLOCKS_DIR . '../spectra-pro/spectra-pro.php' ),
				'spectra_blocks_pro_licensing'     => file_exists( SPECTRA_BLOCKS_DIR . '../spectra-pro/admin/license-handler.php' ),
				...
				'spectra_blocks_website'           => array(
					...
					'rssFeedURL'             => esc_url( SPECTRA_BLOCKS_URI . '/whats-new/feed/' ),
				...
				// Alternate key name for backward compatibility
				...
				'plugin_manager_nonce'      => wp_create_nonce( 'spectra_blocks_plugin_manager_nonce' ),
				...
				'auto_block_recovery_nonce' => wp_create_nonce( 'uag_auto_block_recovery' ),
				'blocks_editor_spacing_nonce' => wp_create_nonce( 'uag_blocks_editor_spacing' ),
				'btn_inherit_from_theme_nonce' => wp_create_nonce( 'uag_btn_inherit_from_theme' ),
				'check_beta_update_available_nonce' => wp_create_nonce( 'uag_check_beta_update_available' ),
				'container_global_elements_gap_nonce' => wp_create_nonce( 'uag_container_global_elements_gap' ),
				'container_global_padding_nonce' => wp_create_nonce( 'uag_container_global_padding' ),
				'content_width_nonce' => wp_create_nonce( 'uag_content_width' ),
				'copy_paste_nonce' => wp_create_nonce( 'uag_copy_paste' ),
				'dynamic_content_mode_nonce' => wp_create_nonce( 'uag_dynamic_content_mode' ),
				'enable_beta_updates_nonce' => wp_create_nonce( 'uag_enable_beta_updates' ),
				'enable_bsf_analytics_option_nonce' => wp_create_nonce( 'uag_enable_bsf_analytics_option' ),
				'enable_file_generation_nonce' => wp_create_nonce( 'uag_enable_file_generation' ),
				'enable_legacy_design_library_nonce' => wp_create_nonce( 'uag_enable_legacy_design_library' ),
				'enable_on_page_css_button_nonce' => wp_create_nonce( 'uag_enable_on_page_css_button' ),
				'fetch_pages_nonce' => wp_create_nonce( 'uag_fetch_pages' ),
				'fse_font_globally_delete_nonce' => wp_create_nonce( 'uag_fse_font_globally_delete' ),
				'license_activation_nonce' => wp_create_nonce( 'uag_license_activation' ),
				'license_deactivation_nonce' => wp_create_nonce( 'uag_license_deactivation' ),
				'load_gfonts_locally_nonce' => wp_create_nonce( 'uag_load_gfonts_locally' ),
				'load_select_font_globally_nonce' => wp_create_nonce( 'uag_load_select_font_globally' ),
				'preload_local_fonts_nonce' => wp_create_nonce( 'uag_preload_local_fonts' ),
				'pro_activate_nonce' => wp_create_nonce( 'uag_pro_activate' ),
				'regenerate_assets_nonce' => wp_create_nonce( 'uag_regenerate_assets' ),
				'select_font_globally_nonce' => wp_create_nonce( 'uag_select_font_globally' ),
				'update_beta_plugin_nonce' => wp_create_nonce( 'uag_update_beta_plugin' ),
				'visibility_mode_nonce' => wp_create_nonce( 'uag_visibility_mode' ),
				'visibility_page_nonce' => wp_create_nonce( 'uag_visibility_page' ),
				// Editor enhancements nonces
				'collapse_panels_nonce' => wp_create_nonce( 'uag_collapse_panels' ),
				'enable_header_titlebar_nonce' => wp_create_nonce( 'uag_enable_header_titlebar' ),
				'enable_quick_action_sidebar_nonce' => wp_create_nonce( 'uag_enable_quick_action_sidebar' ),
				'enable_templates_button_nonce' => wp_create_nonce( 'uag_enable_templates_button' ),
				// Block settings nonces
				'insta_linked_accounts_nonce' => wp_create_nonce( 'uag_insta_linked_accounts' ),
				'insta_refresh_all_tokens_nonce' => wp_create_nonce( 'uag_insta_refresh_all_tokens' ),
				'recaptcha_site_key_v2_nonce' => wp_create_nonce( 'uag_recaptcha_site_key_v2' ),
				'recaptcha_site_key_v3_nonce' => wp_create_nonce( 'uag_recaptcha_site_key_v3' ),
				'recaptcha_secret_key_v2_nonce' => wp_create_nonce( 'uag_recaptcha_secret_key_v2' ),
				'recaptcha_secret_key_v3_nonce' => wp_create_nonce( 'uag_recaptcha_secret_key_v3' ),
				'load_font_awesome_5_nonce' => wp_create_nonce( 'uag_load_font_awesome_5' ),
			)
		);

		// If the Zip AI Assets is available, add the Zip AI localizations.
		if ( is_array( $localize )
			&& class_exists( '\SpectraZipAI\Classes\Helper' )
			&& class_exists( '\SpectraZipAI\Classes\Module' )
		...
		do_action( 'spectra_blocks_admin_prerequisite_scripts' );
	...
	public function settings_app_scripts( $localize ) {
		...
		$build_path        = SPECTRA_ADMIN_DIR . 'assets/build/';
		$build_url         = SPECTRA_ADMIN_URL . 'assets/build/';
		...
		wp_set_script_translations( $handle, 'spectra', SPECTRA_BLOCKS_DIR . 'languages' );
