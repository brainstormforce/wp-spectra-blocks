...
const ClearV3Cache = () => {
	...
	const [ clearCacheState, setClearCacheState ] = useState( false );
	...
	const clearCache = () => {
		...
		getApiFetchData
			.then( ( _data ) => {
				if ( _data.success ) {
					dispatch( { type: 'UPDATE_SETTINGS_SAVED_NOTIFICATION', payload: 'Cache Cleared Successfully!' } );
					setClearCacheState( false );
					setIsModalOpen( false );
				} else {
					setClearCacheState( false );
					dispatch( { type: 'UPDATE_SETTINGS_SAVED_NOTIFICATION', payload: 'Failed to clear cache!' } );
				}
			} )
			.catch( () => {
				setClearCacheState( false );
				dispatch( { type: 'UPDATE_SETTINGS_SAVED_NOTIFICATION', payload: 'Error clearing cache!' } );
			} );
	};
	...
	// Don't render if CSS cache is disabled
	if ( 'enabled' === disableCSSCache ) {
		return null;
	}
