...
const RollBack = () => {
	...
	return (
		<>
			<SettingsItem
				title={ __( 'Rollback to a Previous Version', 'spectra-blocks' ) }
				settingText={ sprintf(
					/* translators: %s: current plugin version */
					__(
						'Experiencing an issue with Spectra Blocks version %s? Rollback to a previous version to help troubleshoot the issue.',
						'spectra-blocks'
					),
			...
			<RollBackConfirmPopup
				{ ...{
					...
					popupContent: {
						title: __( 'Rollback to a Previous Version', 'spectra-blocks' ),
						description: sprintf(
							/* translators: %1$s: selected plugin version */
							__(
								'Are you sure you want to rollback to Spectra Blocks v%1$s?',
								'spectra-blocks'
							),
							previousVersionSelect
						),
