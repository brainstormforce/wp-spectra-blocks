...
const InheritFromThemeRender = () => {
	...
	const updateEnableInheritFromTheme = () => {
		...
		const data = {
			security: spectra_blocks_react.btn_inherit_from_theme_nonce,
			value: status,
		};

		const getApiDataFetch = getApiData( {
			url: spectra_blocks_react.ajax_url,
			action: 'spectra_blocks_btn_inherit_from_theme',
			data,
		} );
	...
	return (
		<>
			<SettingsItem
				title={ __( 'Button - Inherit From Theme', 'spectra-blocks' ) }
				settingText={ __(
